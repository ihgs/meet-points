...
export function ResultCardCompact({ result, rank, maxTime, isFirst, style }: ResultCardCompactProps) {
  ...
  return (
    <div
      className="bg-bg-card border border-line rounded-lg transition-colors hover:border-line-2 animate-card-in"
      style={style}
    >
      <div
      ...
      >
        <div className={`size-7 rounded-full grid place-items-center font-num font-semibold text-[13px] ${isFirst ? 'text-accent-fg bg-accent-soft' : 'text-fg-2 bg-bg-soft'}`}>
          {rank}
        </div>
        <div>
